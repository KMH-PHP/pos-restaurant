...
const CustomerInfo = () => {
  ...
  return (
    ...
    <div className='flex flex-col items-start'>
      ...
      <p className='text-xs text-[#ababab] font-medium mt-1'>#{customerData.orderId || 
        "N/A"} / Dine in</p>
      <p className='text-xs text-[#ababab] font-medium mt-2'>{formatDate(dateTime)} </p>
    </div>
    <button className='bg-[#f6b100] p-3 text-xl font-bold rounded-lg'>{getAvatarName(customerData.customerName) || "CN"
      }</button>
  </div>
  )
}
